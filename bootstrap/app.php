...
if (PHP_VERSION_ID < 80200) {
    $errorMessage = "Error: IndieInABox requires PHP version 8.2.0 or higher. Your current PHP version is " . PHP_VERSION . ". Please upgrade your PHP installation.";
    if (PHP_SAPI === 'cli') {
...
<head>
    ...
    <style>
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f7fafc; color: #2d3748; padding: 2rem; display: flex; align-items: center; justify-content: center; min-height: 100vh; margin: 0; }
        .card { background: white; padding: 2.5rem; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06); max-width: 500px; width: 100%; border-top: 4px solid #e53e3e; }
        h1 { color: #e53e3e; font-size: 1.5rem; margin-top: 0; }
        p { line-height: 1.6; margin-bottom: 1.5rem; }
        code { background-color: #edf2f7; padding: 0.2rem 0.4rem; border-radius: 4px; font-family: monospace; font-size: 0.9em; }
    </style>
</head>
<body>
    <div class='card'>
        ...
        <p>Your current PHP version is <code>" . htmlspecialchars(PHP_VERSION) . "</code> which is too old. Please upgrade PHP to run this application.</p>
    </div>
</body>
</html>";
        exit(1);
