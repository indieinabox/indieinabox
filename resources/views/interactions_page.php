...
<body>
    ...
    <main>
        <article class="h-entry">
            <h1><?= \Indieinabox\Localization\Translator::translate('Interactions on') ?>: <a href="<?= $page->relpath ?><?= $page->slug ?>"><?= htmlspecialchars($page->title) ?></a></h1>

            <?php if (count($likes) > 0): ?>
                <section id="likes" style="margin-top: 2em;">
                    <h2><?= count($likes) ?> <?= \Indieinabox\Localization\Translator::translatePlural('Like', 'Likes', count($likes)) ?></h2>
            ...
            <?php if (count($reposts) > 0): ?>
                <section id="reposts" style="margin-top: 2em;">
                    <h2><?= count($reposts) ?> <?= \Indieinabox\Localization\Translator::translatePlural('Repost', 'Reposts', count($reposts)) ?></h2>
                    <ul style="list-style: none; padding-left: 0;">
            ...
            <?php if (count($replies) > 0): ?>
                <section id="replies" style="margin-top: 2em;">
                    <h2><?= count($replies) ?> <?= \Indieinabox\Localization\Translator::translatePlural('Reply', 'Replies', count($replies)) ?></h2>
                            ...
                            <div class="p-comment h-cite" id="reply-<?= md5($reply['url']) ?>" style="margin-bottom: 1.5em; padding-left: 10px; border-left: 2px solid var(--accent);">
                                <div style="margin-bottom: 0.3em;">
                                    ...
                                    <a href="<?= $replyUrl ?>" style="margin-left: 10px; font-size: 0.85em; opacity: 0.7;">
                                        <?= \Indieinabox\Localization\Translator::translate('Permalink') ?>
                                    </a>
                                </div>
